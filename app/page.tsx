...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/Mehndi.mp3", { method: "HEAD" }).then((r) => setMusicAvailable(r.ok)).catch(() => setMusicAvailable(false));
    fetch("/mehndi.mp4", { method: "HEAD" }).then((r) => setVideoAvailable(r.ok)).catch(() => setVideoAvailable(false));
  }, []);
  useEffect(() => {
  ...
  return <main>
    <section id="home" className="hero">
      {videoAvailable && (<video className="hero-video" autoPlay loop muted playsInline poster="/mehndi-botanical-backdrop.png"><source src="/" type="video/mp4" /></video>)}
      <div className="hero-overlay" /><div className="hero-content">
        <p className="hero-event">Mehndi</p><p className="hero-host">MR. &amp; MRS. AZHAR BASHIR</p><p className="hero-copy">CORDIALLY INVITE YOU<br />TO CELEBRATE THE MEHNDI NIGHT<br />OF THEIR BELOVED SON</p>
        <h1>Muhammad Zeeshan Azhar Malik</h1><p className="program">Programme In-sha-Allah</p><div className="hero-date"><span /><b>Friday</b><span /></div><p>6th &nbsp;|&nbsp; NOVEMBER &nbsp;|&nbsp; 2026</p><p>MEHNDI CELEBRATION</p>
      </div><a className="scroll-cue" href="#countdown">⌄<small>Scroll to discover</small></a>
      <audio ref={audio} autoPlay loop preload="auto" src="/Mehndi.mp3" onEnded={() => setPlaying(false)} />
      <button className="music-button" onClick={toggleMusic} aria-label={playing ? "Pause music" : "Play music"}>{playing ? "Ⅱ" : "▶"}<span>{playing ? "Music on" : "Play music"}</span></button>
    </section>
    ...
    <section id="details" className="details section"><p className="kicker">Mehndi Night</p><h2>Event Details</h2><div className="details-grid"><article><h3>West Canal Road</h3><p className="detail">◷ &nbsp; 7:00 PM onwards</p><p className="detail">⌖ &nbsp; Farooqabad, Mansoorabad</p><p className="note">Kindly ensure your timely arrival to help us celebrate a beautiful evening together.</p><a className="directions" href="https://maps.app.goo.gl/7Am4wkHEruW9SegB8" target="_blank" rel="noreferrer">Get directions ↗</a></article><div className="map"><iframe src="https://maps.google.com/maps?q=West+Canal+Road,+Farooqabad,+Mansoorabad&t=&z=13&ie=UTF8&iwloc=&output=embed" loading="lazy" title="Event venue map" /></div></div><p className="thanks">We can&apos;t wait to celebrate with you!</p></section>
